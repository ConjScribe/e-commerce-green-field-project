...
app.put('/addToCart',function(req,res){
	console.log(JSON.parse(Object.keys(req.body)[0]).cart)
	var i = JSON.parse(Object.keys(req.body)[0]).cart
	models.User.findById(user._id,function(err,updatedUser){
		if(err){
			console.log(err)
		}else{
			updatedUser.items.push(i);
			console.log(updatedUser.items)
			res.redirect('/')
			updatedUser.save(function(err,updUser){
				if(err){
					console.log(err)
				}else{
					console.log('hello')
